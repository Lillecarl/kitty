...
base64_encode_into(PyObject UNUSED *self, PyObject *args) {
    int add_padding = 0;
    RAII_PY_BUFFER(view);
    RAII_PY_BUFFER(output);
    if (!PyArg_ParseTuple(args, "s*w*|i", &view, &output, &add_padding)) return NULL;
    size_t sz = required_buffer_size_for_base64_encode(view.len);
    if (output.len < (ssize_t)sz) {
        PyErr_SetString(PyExc_TypeError, "output buffer too small");
        return NULL;
    }
...
split_into_graphemes(PyObject UNUSED *self, PyObject *src) {
    if (!PyUnicode_Check(src)) {
        PyErr_SetString(PyExc_TypeError, "must provide a unicode string");
        return NULL;
    }
    int kind = PyUnicode_KIND(src);
    char *data = PyUnicode_DATA(src);
...
typedef struct StreamingBase64Decoder {
    PyObject_HEAD struct base64_state state;
...
StreamingBase64Decoder_init(PyObject *s, PyObject *args, PyObject *kwds UNUSED) {
    if (PyTuple_GET_SIZE(args)) {
        PyErr_SetString(PyExc_TypeError, "constructor takes no arguments");
        return -1;
    }
...
StreamingBase64Decoder_decode(StreamingBase64Decoder *self, PyObject *a) {
    ...
    if (!ans) return NULL;
    int ret;
    Py_BEGIN_ALLOW_THREADS ret = base64_stream_decode(&self->state, data.buf, data.len, PyBytes_AS_STRING(ans), &sz);
...
StreamingBase64Decoder_decode_into(StreamingBase64Decoder *self, PyObject *const *args, Py_ssize_t nargs) {
    if (nargs != 2) {
        PyErr_SetString(PyExc_TypeError, "constructor takes exactly two arguments");
        return NULL;
    }
    RAII_PY_BUFFER(data);
    ...
    if ((Py_ssize_t)sz > data.len) {
        PyErr_SetString(PyExc_BufferError, "output buffer too small");
        return NULL;
    }
    int ret;
    Py_BEGIN_ALLOW_THREADS ret = base64_stream_decode(&self->state, src.buf, src.len, data.buf, &sz);
    Py_END_ALLOW_THREADS if (!ret) {
        StreamingBase64Decoder_reset_(self);
        PyErr_SetString(PyExc_ValueError, "Invalid base64 input data");
        return NULL;
    }
    if (self->state.eof) StreamingBase64Decoder_reset_(self);
    else self->needs_more_data = true;
    return PyLong_FromSize_t(sz);
}
...
static PyTypeObject StreamingBase64Decoder_Type = {
    PyVarObject_HEAD_INIT(NULL, 0).tp_name = "kitty.fast_data_types.StreamingBase64Decoder",
...
StreamingBase64Encoder_encode(StreamingBase64Decoder *self, PyObject *a) {
    ...
    RAII_PyObject(ans, PyBytes_FromStringAndSize(NULL, sz));
    if (!ans) return NULL;
    Py_BEGIN_ALLOW_THREADS base64_stream_encode(&self->state, data.buf, data.len, PyBytes_AS_STRING(ans), &sz);
    Py_END_ALLOW_THREADS if (_PyBytes_Resize(&ans, sz) != 0) return NULL;
    return Py_NewRef(ans);
}
...
StreamingBase64Encoder_encode_into(StreamingBase64Decoder *self, PyObject *const *args, Py_ssize_t nargs) {
    ...
    Py_BEGIN_ALLOW_THREADS base64_stream_encode(&self->state, src.buf, src.len, data.buf, &sz);
    Py_END_ALLOW_THREADS return PyLong_FromSize_t(sz);
}
...
StreamingBase64Encoder_reset(StreamingBase64Decoder *self, PyObject *args UNUSED) {
    ...
    base64_stream_encode_init(&self->state, 0);
    if (!self->add_trailing_bytes) {
        while (sz && trailer[sz - 1] == '=') sz--;
    }
    return PyBytes_FromStringAndSize(trailer, sz);
}

static PyTypeObject StreamingBase64Encoder_Type = {
    PyVarObject_HEAD_INIT(NULL, 0).tp_name = "kitty.fast_data_types.StreamingBase64Encoder",
    .tp_basicsize = sizeof(StreamingBase64Decoder),
    .tp_flags = Py_TPFLAGS_DEFAULT,
...
put_tty_in_raw_mode(int fd, const struct termios *termios_p, bool read_with_timeout, int optional_actions) {
    ...
    if (tcsetattr(fd, optional_actions, &raw_termios) != 0) {
...
open_tty(PyObject *self UNUSED, PyObject *args) {
    ...
    if (!put_tty_in_raw_mode(fd, termios_p, read_with_timeout != 0, optional_actions)) {
        free(termios_p);
        return NULL;
    }
...
#define add_digit(base)                                           \
    ...
    while (idx < PyUnicode_GET_LENGTH(src)) {
        Py_UCS4 ch = PyUnicode_READ(kind, data, idx);
        idx++;
        switch (state) {
            case NORMAL: {
            ...
            case CONTROL_CHAR:
                ...
                break;
            case HEX_DIGIT: {
                if (hex_digit_idx < max_num_hex_digits && (('0' <= ch && ch <= '9') || ('a' <= ch && ch <= 'f') || ('A' <= ch && ch <= 'F')))
                    add_digit(16) else {
                        write_digits(16);
                        idx--;
                    }
            ...
            case OCT_DIGIT: {
                if ('0' <= ch && ch <= '7' && hex_digit_idx < max_num_hex_digits) add_digit(16) else {
                        write_digits(8);
                        idx--;
                    }
            }; break;
            case PREV_ESC: {
                ...
                switch (ch) {
                    ...
                    case 'b': w(8); break;
                    case 'c':
                        if (idx < PyUnicode_GET_LENGTH(src)) {
                            state = CONTROL_CHAR;
                        } else {
                            w('\\');
                            w(ch);
                    ...
                    case 'v': w(11); break;
                    case 'x':
                        max_num_hex_digits = 2;
                        hex_digit_idx = 0;
                        state = HEX_DIGIT;
                        break;
                    case 'u':
                        max_num_hex_digits = 4;
                        hex_digit_idx = 0;
                        state = HEX_DIGIT;
                        break;
                    case 'U':
                        max_num_hex_digits = 8;
                        hex_digit_idx = 0;
                        state = HEX_DIGIT;
                        break;
                    case '0' ... '7':
                        max_num_hex_digits = 3;
                        hex_digits[0] = ch;
                        hex_digit_idx = 1;
                        state = OCT_DIGIT;
                        break;
...
c0_replace_unicode(PyObject *input) {
    ...
    for (Py_ssize_t i = 0; i < PyUnicode_GET_LENGTH(input); i++) {
        ...
        switch (ch) {
            case C0_EXCEPT_NL_SPACE_TAB: ch += 0x2400; changed = true;
        }
...
py_char_props_for(PyObject *self UNUSED, PyObject *ch) {
    if (!PyUnicode_Check(ch) || PyUnicode_GET_LENGTH(ch) != 1) {
        PyErr_SetString(PyExc_TypeError, "must supply a single character");
        return NULL;
    }
...
#define B(x) #x, cp.x ? Py_True : Py_False
    return Py_BuildValue(
        "{si sO sB sB ss sO sO}",
        "width",
        wcwidth_std(cp),
        B(is_extended_pictographic),
        "grapheme_break",
        cp.grapheme_break,
        "indic_conjunct_break",
        cp.indic_conjunct_break,
        "category",
        char_category(cp),
        B(is_emoji),
        B(is_emoji_presentation_base));
#undef B
}
...
expanduser(PyObject *self UNUSED, PyObject *path) {
    if (!PyUnicode_Check(path)) {
        PyErr_SetString(PyExc_TypeError, "path must a string");
        return NULL;
    }
    char buf[PATH_MAX + 1];
...
set_uint_at_address(PyObject *self UNUSED, PyObject *const *args, Py_ssize_t nargs) {
    if (nargs != 2) {
        PyErr_SetString(PyExc_TypeError, "set_uint_at_address() requires exactly 2 arguments");
        return NULL;
    }
