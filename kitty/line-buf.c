...
linebuf_set_last_char_as_continuation(LineBuf *self, index_type y, bool continued) {
    if (y < self->ynum) { cpu_lineptr(self, self->line_map[y])[self->xnum - 1].next_char_was_wrapped = continued; }
}
...
#define set_attribute_doc "set_attribute(which, val) -> Set the attribute on all cells in the line."
    ...
    for (index_type y = 0; y < self->ynum; y++) {
        if (!set_named_attribute_on_line(gpu_lineptr(self, y), which, val, self->xnum)) {
            PyErr_SetString(PyExc_KeyError, "Unknown cell attribute");
            return NULL;
        }
        self->line_attrs[y].has_dirty_text = true;
    }
...
#define dirty_lines_doc "dirty_lines() -> Line numbers of all lines that have dirty text."
    ...
    for (index_type i = 0; i < self->ynum; i++) {
        if (self->line_attrs[i].has_dirty_text) { PyList_Append(ans, PyLong_FromUnsignedLong(i)); }
    }
    return ans;
}
...
create_line_copy_inner(LineBuf *self, index_type y) {
    ...
    src.xnum = self->xnum;
    line->xnum = self->xnum;
    if (!allocate_line_storage(line, 0)) {
        Py_CLEAR(line);
...
#error This implementation is incorrect for BLANK_CHAR != 0
    ...
    CPUCell *c = cpu_lineptr(self, ym);
    GPUCell *g = gpu_lineptr(self, ym);
...
#define rewrap_doc                                                                                                                                             \
    "rewrap(new_screen) -> Fill up new screen (which can have different size to this screen) with as much of the contents of this screen as will fit. Return " \
    "lines that overflow."

static PyMethodDef methods[] = {
    METHOD(line, METH_O) METHOD(clear_line, METH_O) METHOD(copy_old, METH_O) METHOD(copy_line_to, METH_VARARGS) METHOD(create_line_copy, METH_O)
        METHOD(rewrap, METH_VARARGS) METHOD(clear, METH_NOARGS) METHOD(as_ansi, METH_O) METHODB(as_text, METH_VARARGS),
    METHOD(set_attribute, METH_VARARGS) METHOD(set_continued, METH_VARARGS)
        METHOD(dirty_lines, METH_NOARGS){"index", (PyCFunction)pyw_index, METH_VARARGS, NULL},
    METHOD(reverse_index, METH_VARARGS) METHOD(insert_lines, METH_VARARGS) METHOD(delete_lines, METH_VARARGS)
        METHOD(is_continued, METH_O){NULL, NULL, 0, NULL} /* Sentinel */
};

static PyMemberDef members[] = {
    {"xnum", T_UINT, offsetof(LineBuf, xnum), READONLY, "xnum"}, {"ynum", T_UINT, offsetof(LineBuf, ynum), READONLY, "ynum"}, {NULL} /* Sentinel */
};

PyTypeObject LineBuf_Type = {
    PyVarObject_HEAD_INIT(NULL, 0).tp_name = "fast_data_types.LineBuf",
...
copy_old(LineBuf *self, PyObject *y) {
    if (!PyObject_TypeCheck(y, &LineBuf_Type)) {
        PyErr_SetString(PyExc_TypeError, "Not a LineBuf object");
        return NULL;
    }
    LineBuf *other = (LineBuf *)y;
    if (other->xnum != self->xnum) {
        PyErr_SetString(PyExc_ValueError, "LineBuf has a different number of columns");
        return NULL;
    }
...
alloc_linebuf(unsigned int lines, unsigned int columns, TextCache *tc) {
    return alloc_linebuf_(&LineBuf_Type, lines, columns, tc);
}
