...
last_focused_os_window_id(void) {
    ...
    for (size_t i = 0; i < global_state.num_os_windows; i++) {
        ...
        if (w->last_focused_counter > max_fc_count) {
            ans = w->id;
            max_fc_count = w->last_focused_counter;
        }
    }
    return ans;
}
...
send_bgimage_to_gpu(BackgroundImageLayout layout, BackgroundImage *bgimage) {
    ...
    switch (layout) {
        ...
        case TILING: r = REPEAT_DEFAULT; break;
    }
    bgimage->texture_id = 0;
    size_t delta = bgimage->mmap_size ? bgimage->mmap_size - ((size_t)4) * bgimage->width * bgimage->height : 0;
    send_image_to_gpu(&bgimage->texture_id, bgimage->bitmap + delta, bgimage->width, bgimage->height, false, true, OPT(background_image_linear), r);
...
free_global_background_images(bool release_texture) {
    if (global_state.background_images.images) {
        for (size_t i = 0; i < global_state.background_images.count; i++) free_bgimage(&global_state.background_images.images[i], release_texture);
...
ensure_background_images_generation(bool release_texture) {
    ...
    if (OPT(background_images).count) {
        global_state.background_images.images = calloc(OPT(background_images).count, sizeof(global_state.background_images.images[0]));
...
background_image_for_os_window(OSWindow *w) {
    ...
    if (w->background_image.global_bg_images_idx && (ans = global_background_image(w->background_image.global_bg_images_idx))) return ans;
...
attach_window(id_type os_window_id, id_type tab_id, id_type id) {
    ...
    for (size_t i = 0; i < detached_windows.num_windows; i++) {
        if (detached_windows.windows[i].id == id) {
            ...
            if (w->render_data.screen->cell_size.width != osw->fonts_data->fcm.cell_width ||
            ...
            screen_visibility_changed(w->render_data.screen, is_os_window_potentially_visible(osw) && t == osw->active_tab && w->visible);
            break;
        }
    }
    END_WITH_TAB;
...
remove_os_window(id_type os_window_id) {
    ...
    if (found) {
        ...
        if (global_state.num_os_windows == 0) {
            ...
            global_state.layers_render_texture.width = 0;
            global_state.layers_render_texture.height = 0;
        }
    }
    return found;
}
...
pyreorder_tabs(PyObject *self UNUSED, PyObject *args) {
    ...
    WITH_OS_WINDOW(os_window_id)
    if (PyTuple_GET_SIZE(args) != (Py_ssize_t)(os_window->num_tabs + 1)) {
        PyErr_SetString(PyExc_ValueError, "number of tabs not correct");
        return NULL;
    }
    ...
    for (Py_ssize_t i = 1; i < PyTuple_GET_SIZE(args); i++) {
        ...
        for (size_t t = 0; t < os_window->num_tabs; t++) {
            if (os_window->tabs[t].id == tab_id) {
                ...
                found = true;
                if (used[t]) {
                    PyErr_SetString(PyExc_ValueError, "duplicate tab ids found");
                    return NULL;
                }
                used[t] = 1;
                break;
        ...
        if (!found) {
            PyErr_SetString(PyExc_ValueError, "tab id not found");
            return NULL;
        }
    }
    free(os_window->tabs);
            ...
            for (size_t w = 0; w < qtab->num_windows; w++) {
                ...
                if (window->id == window_id) {
                    ...
                    if ((window->render_data.screen && (force || osw->is_focused)) || update_focus > 0) {
                        ...
                        if (update_focus >= 0 && window->render_data.screen) { update_ime_position(window, window->render_data.screen); }
...
PYWRAP0(next_window_id) { return PyLong_FromUnsignedLongLong(global_state.window_id_counter + 1); }

PYWRAP0(last_focused_os_window_id) { return PyLong_FromUnsignedLongLong(last_focused_os_window_id()); }
...
PYWRAP1(set_options) {
    PyObject *opts;
    int is_wayland = 0, debug_rendering = 0, debug_font_fallback = 0, is_server = 0, is_client = 0;
    PA("O|ppppp", &opts, &is_wayland, &debug_rendering, &debug_font_fallback, &is_server, &is_client);
    if (opts == Py_None) {
    ...
    global_state.is_client = is_client ? true : false;
...
init_window_render_data(WindowRenderData *d, const WindowGeometry g, Screen *screen) {
    d->geometry = g;
    Py_CLEAR(d->screen);
    d->screen = (Screen *)Py_NewRef(screen);
}
...
static PyStructSequence_Field region_fields[] = {{"left", ""}, {"top", ""}, {"right", ""}, {"bottom", ""}, {"width", ""}, {"height", ""}, {NULL, NULL}};
...
PYWRAP0(current_application_quit_request) { return Py_BuildValue("i", global_state.quit_request); }
...
PYWRAP1(set_window_padding) {
    ...
    WITH_WINDOW(os_window_id, tab_id, window_id);
    window->padding.left = left;
    window->padding.top = top;
    window->padding.right = right;
    window->padding.bottom = bottom;
    END_WITH_WINDOW;
    Py_RETURN_NONE;
}
...
#define S(name) &(g.spaces.name)
    ...
    window->size_mismatch_padding.left = cl;
    window->size_mismatch_padding.top = ct;
    window->size_mismatch_padding.right = cr;
    window->size_mismatch_padding.bottom = cb;
    END_WITH_WINDOW;
    Py_RETURN_NONE;
#undef B
#undef S
...
PYWRAP1(update_window_visibility) {
    ...
    if (window->render_data.screen) {
        screen_visibility_changed(window->render_data.screen, is_os_window_potentially_visible(osw) && t == osw->active_tab && window->visible);
    }
    END_WITH_WINDOW;
    Py_RETURN_NONE;
}
...
PYWRAP1(get_os_window_size) {
    ...
    return Py_BuildValue(
        "{si si si si sf sf sd sd sI sI sO}",
        "width",
        width,
        "height",
        height,
        ...
        cell_height,
        "is_layer_shell",
        os_window->is_layer_shell ? Py_True : Py_False);
    END_WITH_OS_WINDOW
    Py_RETURN_NONE;
}
...
#define P(name)                                                         \
    ...
    P(bell_border_color);
    P(tab_bar_background);
    P(tab_bar_margin_color);
    P(macos_titlebar_color);
    P(wayland_titlebar_color);
    P(scrollbar_handle_color);
    P(scrollbar_track_color);
    if (configured) {
        P(background);
        P(url_color);
    }
    ...
    if (screen) {
        ...
        if (!is_vertical) {
            ok = get_line_edge_colors_at_row(
                screen, screen->cursor->y, &os_window->tab_bar_edge_color.left, &os_window->tab_bar_edge_color.right, &left_is_default, &right_is_default);
        } else {
        ...
        if (ok) { return Py_BuildValue("OO", left_is_default ? Py_True : Py_False, right_is_default ? Py_True : Py_False); }
    }
    END_WITH_OS_WINDOW
    Py_RETURN_NONE;
}
...
pyset_background_image(PyObject *self UNUSED, PyObject *args, PyObject *kw) {
    ...
    int configured = 0;
    char *png_data = NULL;
    Py_ssize_t png_data_size = 0;
    int global_index = -1;
    int is_increment = 0;
    static char *kwds[] = {
        "path", "os_window_ids", "configured", "layout_name", "png_data", "linear", "tint", "tint_gaps", "global_index", "is_increment", NULL};
...
PYWRAP0(destroy_global_data) {
    Py_CLEAR(global_state.boss);
    free(global_state.os_windows);
    global_state.os_windows = NULL;
    Py_RETURN_NONE;
}
    ...
    if (window->window_logo.instance != NULL && window->window_logo.id && !window->window_logo.using_default) {
        ...
        if (wl != NULL && wl->load_from_disk_ok) {
            ...
            if (path) {
                ...
                return Py_BuildValue("sfN", path, window->window_logo.alpha, Py_BuildValue("ffff", p->image_x, p->image_y, p->canvas_x, p->canvas_y));
            }
        }
    }
    END_WITH_WINDOW;
...
PYWRAP1(set_window_logo) {
    ...
    ok = set_window_logo(
        window,
        path,
        PyObject_IsTrue(position) ? bganchor(position) : OPT(window_logo_position),
        (0 <= alpha && alpha <= 1) ? alpha : OPT(window_logo_alpha),
        false,
        png_data,
        png_data_size);
    END_WITH_WINDOW;
...
PYWRAP1(add_window) {
    ...
    id_type a, b;
    PA("KKO", &a, &b, &title);
    return PyLong_FromUnsignedLongLong(add_window(a, b, title));
}
PYWRAP0(current_os_window) {
...
get_mouse_data_for_window(PyObject *self UNUSED, PyObject *args) {
    ...
    return Py_BuildValue(
        "{sI sI sO}",
        "cell_x",
        window->mouse_pos.cell_x,
        "cell_y",
        window->mouse_pos.cell_y,
        "in_left_half_of_cell",
        window->mouse_pos.in_left_half_of_cell ? Py_True : Py_False);
    END_WITH_WINDOW
    Py_RETURN_NONE;
}
...
request_callback_with_thumbnail(PyObject *self UNUSED, PyObject *args) {
    ...
    const char *callback;
    int include_tab_bar = 0, no_scaling = 0;
    double scale = 0.25;
    unsigned max_width = 480;
...
static void
finalize(void) {
    while (detached_windows.num_windows--) { destroy_window(&detached_windows.windows[detached_windows.num_windows]); }
...
#define DPI 96.0
#endif
    global_state.default_dpi.x = DPI;
    global_state.default_dpi.y = DPI;
    global_state.all_window_logos = alloc_window_logo_table();
    if (!global_state.all_window_logos) {
        PyErr_NoMemory();
        return false;
    }
