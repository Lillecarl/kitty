...
typedef struct {
    PyObject_HEAD PyObject *key, *shifted_key, *alternate_key;
...
new_keyevent_object(PyTypeObject *type UNUSED, PyObject *args, PyObject *kw) {
    ...
    if (!PyArg_ParseTupleAndKeywords(
            args, kw, "I|IIiiiiz", kwds, &ev.key, &ev.shifted_key, &ev.alternate_key, &ev.mods, &ev.action, &ev.native_key, &ev.ime_state, &ev.text))
        return NULL;
...
is_modifier_key(const uint32_t key) {
    ...
    switch (key) {
        ...
        case GLFW_FKEY_NUM_LOCK: return true;
...
#endif
        default: return is_modifier_key(key);
    }
}
...
dealloc(PyKeyEvent *self) {
    Py_CLEAR(self->key);
    Py_CLEAR(self->shifted_key);
    Py_CLEAR(self->alternate_key);
    Py_CLEAR(self->mods);
    Py_CLEAR(self->action);
    Py_CLEAR(self->native_key);
    Py_CLEAR(self->ime_state);
    Py_CLEAR(self->text);
...
PyTypeObject PyKeyEvent_Type = {
    PyVarObject_HEAD_INIT(NULL, 0).tp_name = "fast_data_types.KeyEvent",
...
prepare_ime_position_update_event(OSWindow *osw, Window *w, Screen *screen, GLFWIMEUpdateEvent *ev) {
    ...
    ev->cursor.left = left;
    ev->cursor.top = top;
    ev->cursor.width = cell_width;
    ev->cursor.height = cell_height;
}
    ...
    if (size == SEND_TEXT_TO_CHILD) {
        ...
        if (OPT(debug_keyboard)) {
            ...
            for (int ki = 0; ki < size; ki++) {
                if (encoded_key[ki] == 27) {
                    debug("^[ ");
                } else if (encoded_key[ki] == ' ') {
                    debug("SPC ");
                } else if (isprint(encoded_key[ki])) {
                    debug("%c ", encoded_key[ki]);
                } else {
                    debug("0x%x ", encoded_key[ki]);
                }
            }
            debug("\n");
        }
...
on_key_input(const GLFWkeyevent *ev) {
    ...
    if (OPT(debug_keyboard)) {
        if (!key && !native_key && text[0]) {
            ...
            debug(
                "\x1b[33mon_key_input\x1b[m: glfw key: 0x%x native_code: 0x%x action: %s %stext: '%s' state: %d ",
                key,
                native_key,
    ...
    if (!w) {
        debug("no active window, ignoring\n");
        return;
    }
...
pyencode_key_for_tty(PyObject *self UNUSED, PyObject *args, PyObject *kw) {
    ...
    if (!PyArg_ParseTupleAndKeywords(
            args, kw, "I|IIIIIzp", kwds, &key, &shifted_key, &alternate_key, &mods, &action, &key_encoding_flags, &text, &cursor_key_mode))
        return NULL;
...
pyinject_key(PyObject *self UNUSED, PyObject *args, PyObject *kw) {
    ...
    if (os_window_id) {
        ...
        if (!found) {
            PyErr_Format(PyExc_IndexError, "Could not find OS Window with id: %llu", os_window_id);
            return NULL;
        }
    } else {
        if (!global_state.callback_os_window) {
            ...
            if (!found && !global_state.num_os_windows) {
                PyErr_SetString(PyExc_Exception, "No OS Windows available to inject key presses into");
                return NULL;
            }
...
SingleKey_new(PyTypeObject *type, PyObject *args, PyObject *kw) {
    long long key = -1;
    unsigned short mods = 1 << (MOD_BITS + 1);
    int is_native = -1;
...
SingleKey_richcompare(PyObject *self, PyObject *other, int op) {
    if (!PyObject_TypeCheck(other, &SingleKey_Type)) {
        PyErr_SetString(PyExc_TypeError, "Cannot compare SingleKey to other objects");
        return NULL;
    }
...
SingleKey_item(PyObject *o, Py_ssize_t i) {
    ...
    switch (i) {
        case 0: return SingleKey_get_mods(self, NULL);
        case 1: return SingleKey_get_is_native(self, NULL);
        case 2: return SingleKey_get_key(self, NULL);
    }
...
SingleKey_resolve_kitty_mod(SingleKey *self, PyObject *km) {
    if (!(self->key.mods & GLFW_MOD_KITTY)) {
        Py_INCREF(self);
        return (PyObject *)self;
    }
...
static PyTypeObject SingleKey_Type = {
    PyVarObject_HEAD_INIT(NULL, 0).tp_name = "fast_data_types.SingleKey",
